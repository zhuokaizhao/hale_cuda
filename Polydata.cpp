...
void Polydata::setTexture(char *varName, const unsigned char *data, int width, int height, int pixsize)
{
  GLuint curId = loadTextureImage(data, width, height, pixsize);
  _textureIds.push_back(curId);
  unsigned int curTex = _textureIds.size()-1;
  _textureVars[varName] = curTex;
}
...
void Polydata::replaceLastTexture(unsigned char *data, unsigned int width, unsigned int height, unsigned int pixsize)
{
  printf("inside replaceLastTexture()\n");
    switch (_textureIds.back())
    {
      case 0:
        glActiveTexture(GL_TEXTURE0);            
        break;
  ...
  printf("inside replaceLastTexture(), before bindTexture()\n");
  glBindTexture(GL_TEXTURE_2D, _textureIds.back());

  //glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
  // Give the image to OpenGL
  printf("inside replaceLastTexture(), before TexSubImage2D()\n");
  if (pixsize == 3)
    //glTexImage2D(GL_TEXTURE_2D, 0,GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height, GL_RGB, GL_UNSIGNED_BYTE, data);
  else //assume pixsize == 4 otherwise
    //glTexImage2D(GL_TEXTURE_2D, 0,GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE, data);
  printf("inside replaceLastTexture(), finished functions\n");
